 ...
 */
     ...
     * @param {ButtonInteraction} interaction 
     */
    async handleViewVotes(interaction) {
        const customId = interaction.customId;
        const parts = customId.split('_');
...
.setTimestamp();
        ...
        try {
            ...
            const memberData = interaction.guild.members.cache.get(user.id);
            if (!memberData) {
                const embed = new EmbedBuilder()
    ...
    async handlePagination(interaction, customId) {
        const parts = customId.split('_'); // e.g., 'next_list_pups_1155078877803192420_0'
    ...
    async handleAdd(interaction) {
        ...
        // Permission check using the allowedRoles list
        if (!this.allowedRoles.some(roleId => executor.roles.cache.has(roleId))) {
            const embed = new EmbedBuilder()
                .setTitle('Missing Role!')
                .setDescription(`Only these members can use this command:\n${this.allowedRoles.map(roleId => `<@&${roleId}>`).join('\n')}`)
                .setColor(0x980e00)
