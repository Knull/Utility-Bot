// handlers/tickets/handleClaimTicketButton.js
...
module.exports = async function handleClaimTicketButton(interaction, client) {
    try {
        ...
        // Update ticket status and claimed_by
        await pool.execute(
        ...
        exportChatLogs(config.token, channel.id, outputFile, limit, tzInfo, militaryTime, async (err, filePath) => {
            if (err) {
                ...
                return interaction.followUp({ content: 'Failed to claim the ticket.', ephemeral: true });
            }
